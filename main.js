...
server.get('/dashboard/presets.html', (req, res) => {
    ...
    server.post('/api/obs/focus', (req, res) => {
        try {
            const cfg = JSON.parse(fs.readFileSync(getDataPath('config.json'), 'utf8'));
            const obsPath = cfg.obsPath || 'C:\\Program Files\\obs-studio\\bin\\64bit\\obs64.exe';
            require('child_process').exec('"' + obsPath + '"');
            res.json({ success: true });
        } catch(e) { res.json({ success: false, error: e.message }); }
    });
...
`;
    ...
    server.listen(3000, () => {
        ...
        // Poll for OBS process — auto-connect when user opens OBS
        setInterval(async () => {
            try {
                ...
                obsConnecting = false; obsLastError = null; obsBroadcastStatus();
            }
        }, 3000);
    });
}
